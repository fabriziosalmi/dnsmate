...
class BindBackupService:
    ...
    async def generate_version_backup(self, session, zone_name: str, version_id: int) -> str:
        """Generate BIND format backup for a specific zone version"""
        ...
        try:
            ...
            if not version:
                raise ValueError(f"Version {version_id} not found for zone {zone_name}")
            
            # Use the stored zone_data and records_data from the version
            zone_data = version.zone_data
            records_data = version.records_data
            
            # Generate BIND zone file content using the version data
            bind_content = self._generate_bind_zone_file(zone_name, zone_data, records_data)
            
            # Add version metadata as comments
            version_header = [
                f"; Zone version backup: {zone_name}",
                f"; Version: {version.version_number}",
                f"; Created: {version.created_at.isoformat()}",
                f"; Description: {version.description or 'No description'}",
                f"; Changes: {version.changes_summary or 'No changes summary'}",
                f"; Generated by DNSMate on {datetime.utcnow().isoformat()}",
                "",
            ]
            
            return "\n".join(version_header) + bind_content
            
        except Exception as e:
            raise ValueError(f"Failed to generate version backup for zone {zone_name} version {version_id}: {str(e)}")
